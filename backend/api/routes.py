...
from backend.agents.interviewer import agent_factory as interviewer_factory
...
from backend.agents.interview_judge import agent_factory as judge_factory
...
from backend.coordinator import workflow_factory
...
async def websocket_endpoint(
    ...
    try: 
        # Start agent session using factory
        result = await interviewer_factory.start_agent_session(session_id, user_id, workflow_id, duration, is_audio)
        
        # Handle different return types (Claude returns session, Gemini returns tuple)
        if isinstance(result, tuple):
            # Gemini mode: returns (live_events, live_request_queue, session)
            live_events, live_request_queue, session = result
            
            # Start tasks
            agent_to_client_task = asyncio.create_task(
                interviewer_factory.agent_to_client_messaging(websocket, live_events, session)
            )
            client_to_agent_task = asyncio.create_task(
                interviewer_factory.client_to_agent_messaging(websocket, live_request_queue, session)
            )
            await asyncio.gather(agent_to_client_task, client_to_agent_task)
        else:
            # Claude mode: returns session only
            session = result
            
            # Start tasks (Claude version doesn't need live_events or live_request_queue)
            agent_to_client_task = asyncio.create_task(
                interviewer_factory.agent_to_client_messaging(websocket, session)
            )
            client_to_agent_task = asyncio.create_task(
                interviewer_factory.client_to_agent_messaging(websocket, session)
            )
            await asyncio.gather(agent_to_client_task, client_to_agent_task)
        ...
        try:
            ...
            # Save transcript (use interviewer factory's save function if available)
            from backend.agents.interviewer.agent import save_transcript
            save_transcript(session)
            print(f"[SAVE]: Transcript saved for session {session_id}")

            # Generate feedback using factory
            await judge_factory.run_judge_from_session(session)
...
async def start_workflow_with_pdf(
    ...
    try:
        ...
        # Start the preparation workflow using factory
        workflow_result = await workflow_factory.run_preparation_workflow(
            user_id=user_id,
