...
def create_preparation_agent(workflow_name: str) -> SequentialAgent:
    """
    Creates a sequential agent for the entire preparation workflow.
    """
    summarizer_agent, search_agent, question_generator_agent, answer_generator_agent = create_fresh_agents()

    # Create a sequential agent to run the two agents in order
    # This replaces the ParallelAgent to avoid async errors.
    agent = SequentialAgent(
        name="SummarizeAndSearch",
        description="Runs resume summarization and industry FAQ search sequentially.",
        sub_agents=[
            summarizer_agent,
            search_agent,
        ]
...
async def run_preparation_workflow(
    ...
    try:
        ...
        preparation_workflow = create_preparation_agent(workflow_name)
