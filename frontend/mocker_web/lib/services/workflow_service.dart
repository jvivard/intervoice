  ...
  Future<List<Workflow>> getWorkflows() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final workflowsData = data['data'];
        
        if (workflowsData == null) {
          debugPrint('⚠️ API returned null data, treating as empty list');
          return [];
        }
        
        if (workflowsData is! List) {
          debugPrint('⚠️ API returned non-list data: $workflowsData');
          return [];
        }
        
        final workflows = (workflowsData as List)
  ...
  Future<List<RecommendedQA>> getRecommendedQAs(String workflowId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (qasData is! List) {
          debugPrint('⚠️ API returned non-list data: $qasData');
          return [];
        }
        
        final qas = (qasData as List)
