...
class InterviewService {
  ...
  Function(String)? _onStreamingUpdate;  // New callback for streaming updates
  Function(String)? _onAudioReceived;  // Callback for audio responses
      ...
      _channel!.stream.listen(
        (message) {
          try {
            ...
            // Handle audio/pcm responses
            if (data.containsKey('mime_type') && data['mime_type'] == 'audio/pcm') {
              final audioData = data['data'] as String;
              debugPrint('[AUDIO] Received audio chunk: ${audioData.length} chars (base64)');
              _onAudioReceived?.call(audioData);
              return;
            }
            
            // accumulate streaming text/plain content
            if (data.containsKey('mime_type') && data['mime_type'] == 'text/plain') {
              _currentStreamingMessage += data['data'] ?? '';  // Append instead of replace
              debugPrint('[STREAMING] Current buffer length: ${_currentStreamingMessage.length}');
            ...
            if (data.containsKey('turn_complete') && data['turn_complete'] == true) {
              if (_currentStreamingMessage.isNotEmpty) {
                debugPrint('[TURN_COMPLETE] Final message length: ${_currentStreamingMessage.length}');
                debugPrint('[TURN_COMPLETE] Full message: $_currentStreamingMessage');
  ...
  void sendAudioData(String base64AudioData) {
    if (_wsConnected && _channel != null) {
      final audioMessage = {
        'mime_type': 'audio/pcm',
        'data': base64AudioData,
      };
      _channel!.sink.add(json.encode(audioMessage));
      debugPrint('📤 Audio data sent via WebSocket: ${base64AudioData.length} chars (base64)');
    } else {
      throw Exception('WebSocket not connected');
    }
  }
